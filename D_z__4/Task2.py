...
# программу для нахождения максимального числа ягод, которое может собрать за один заход собирающий модуль, находясь
# перед некоторым кустом заданной во входном файле грядки.

n = int(input("Введите колличество кустов на грядке : "))
arr = list()
for i in range(n):
    x = int(input(f"Введите колличество ягод на кусте № {i+1}: "))
    arr.append(x)
...
print(f"Колличество ягод на трех соседних самых плодоносных кустах равно : {max(arr_count)}")
